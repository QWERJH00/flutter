...
// 따라서 순서를 변경할 수 있다. set{}처럼 키값으로 찾아감.
  ...
  // 둘다 사용해도 된다. 형식은 (key : value) 이것만 지키면 된다.


  something4();
  something4(age: 20);
  something4(name : "유비");
  something4(age: 40, name: "관우"); // 순서를 지키지 않아도 된다.{}(set성질)이므로
  something4(name: "장비", age: 20); 

  something5('해리포터');
  something5('멀린', age: 20);
 // something5(age: 40, name: "관우"); // 순서변경 안됨.(필수가있어서)
...
/*
  메서드 오벌로딩 : 오버로딩이란 메서드를 정의할 때 동일한 이름으로 정의하되
  매개변수의 개수나 타입을 다르게 해서 서로 다른 메서드처럼 정의하는 기법
  예) myFunc(int a), myFunc(int a, int b), myFunc(double a, String b)
  위 3가지 메서드는 모두 다르다.
*/
// 여러가지 파라미터를 처리하기 위해 오버로딩할 필요 없다. {}옵션
...
// 파라미터의 디폴트 값. int age = 10이기때문에 ?가 빠진다.
void something6(String name, {int age = 10}) {
  print('name : $name age : $age');
}

// 반드시 필요한 파라미터라면 ... 이전 방식 @reguired(옛날버전) 강제로 사용
// 생략하면 에러남.
